...
def _pair_for_non_potential(env: HexagonBasketballEnv, initial_bh) -> Tuple[int, int]:
    ...
    """
    bh = initial_bh if initial_bh is not None else (env.offense_ids[0] if env.offense_ids else 0)
    ...
    with mlflow.start_run(run_id=args.run_id):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if args.use_unified or (not pairs and uni):
                if not uni:
                    raise RuntimeError("No unified policy artifacts found under models/ for this run.")
                ...
                if not pairs:
                    raise RuntimeError("No paired policy artifacts found under models/ for this run.")
                latest_idx = max(pairs.keys())
                pair = pairs[latest_idx]
                offense_path = client.download_artifacts(args.run_id, pair["offense"], temp_dir)
                defense_path = client.download_artifacts(args.run_id, pair["defense"], temp_dir)
            ...
            for _ in range(args.episodes):
                ...
                initial_ball_holder = env.ball_holder if env.ball_holder is not None else (env.offense_ids[0] if env.offense_ids else 0)
                final_info = None
                while not done:
                    if policy_kind == "unified":
                        ...
                        off_action, _ = offense_policy.predict(obs, deterministic=args.deterministic_offense)
                        def_action, _ = defense_policy.predict(obs, deterministic=args.deterministic_defense)
                        full_action = np.zeros(env.n_players, dtype=int)
                        for pid in range(env.n_players):
                            full_action[pid] = off_action[pid] if pid in env.offense_ids else def_action[pid]
                ...
                if action_results.get("shots"):
                    ...
                    if shot_result.get("assist_potential"):
                        potential_assist = 1
                        passer_id = int(shot_result.get("assist_passer_id", initial_ball_holder))
            ...
            try:
                ...
                if len(np.unique(y)) >= 2 and len(y) >= 5:
                    ...
                    try:
                        ...
                        mlflow.log_param("assist_delta_intercept", float(clf.intercept_[0]))
                    except Exception:
                        pass
            ...
            with open(out_path, "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=["delta", "potential_assist", "ball_handler_pct", "teammate_pct"])
                writer.writeheader()
...
def main():
    ...
    parser.add_argument("--episodes", type=int, default=200, help="Number of episodes to simulate.")
    parser.add_argument("--use-unified", action="store_true", help="Use unified policy instead of paired.", default=False)
    parser.add_argument("--deterministic-unified", type=lambda v: str(v).lower() in ["1","true","yes","y","t"], default=False, help="Deterministic actions for unified policy.")
    parser.add_argument("--deterministic-offense", type=lambda v: str(v).lower() in ["1","true","yes","y","t"], default=False, help="Deterministic actions for offense policy (paired).")
    parser.add_argument("--deterministic-defense", type=lambda v: str(v).lower() in ["1","true","yes","y","t"], default=False, help="Deterministic actions for defense policy (paired).")
