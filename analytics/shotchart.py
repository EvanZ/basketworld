...
def main(args):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while not done:
                ...
                if action_results.get("shots"):
                    for shooter_id, shot_res in action_results["shots"].items():
                        # Shooter's cell at the time of the shot (positions remain unchanged for shooter this step)
        ...
        out_path = os.path.join(args.output_dir, f"shotchart_offense_alt_{off_alt_label}.png")
        ...
        _plot_shot_chart(attempts, makes, "Shot Chart", out_path, cmap_name=args.cmap_name)
        print(f"Saved: {out_path}")

        # Log to MLflow
        if not args.no_log_mlflow:
            with mlflow.start_run(run_id=args.run_id):
                mlflow.log_artifact(out_path, artifact_path="shotcharts")
            print("Logged shotchart to MLflow under 'shotcharts/'.")
