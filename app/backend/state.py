...
def get_full_game_state(
    ...
    state = {
        ...
        "pass_interception_model": str(getattr(env, "pass_interception_model", "line") or "line"),
        "pass_passer_pressure_weight": float(getattr(env, "pass_passer_pressure_weight", 0.0)),
        "pass_receiver_pressure_weight": float(getattr(env, "pass_receiver_pressure_weight", 0.0)),
        "pass_lob_lane_multiplier": float(getattr(env, "pass_lob_lane_multiplier", 0.35)),
        "pass_lob_receiver_distance": float(getattr(env, "pass_lob_receiver_distance", 1.0)),
        "pass_speed": float(getattr(env, "pass_speed", 3.5)),
        "defender_reaction_time": float(getattr(env, "defender_reaction_time", 0.35)),
        "defender_speed": float(getattr(env, "defender_speed", 1.25)),
        "defender_reach_radius": float(getattr(env, "defender_reach_radius", 0.65)),
        "reaction_softness": float(getattr(env, "reaction_softness", 0.55)),
        "base_passer_risk": float(getattr(env, "base_passer_risk", 0.06)),
        "passer_pressure_decay": float(getattr(env, "passer_pressure_decay", 1.35)),
        "base_receiver_risk": float(getattr(env, "base_receiver_risk", 0.35)),
        "receiver_alignment_min": float(getattr(env, "receiver_alignment_min", 0.35)),
