...
exec "$PYTHON_BIN" -m basketworld_jax.train.main \
  ...
  dev \
  --mlflow-run-name \
  'skill-only rebound-observation baseline | eval deploy | offense intents | no defense intents | no rebound redistribution | rebound basket position weight 1 | rebound contest radius 2 | rebound skill + lower pass risk | reaction pass model | 5-on-5'
